package com.example.homework2.common.adapter.decorators

import android.graphics.Canvas
import android.view.View
import android.view.ViewGroup
import androidx.recyclerview.widget.RecyclerView
import com.example.homework2.dialog.presentation.ui.adapters.main.delegates.date.ViewHolder

class StickyHeaderItemDecoration : RecyclerView.ItemDecoration() {

    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        val topChild = parent.getChildAt(0) ?: return
        if (parent.getChildAdapterPosition(topChild) == -1) return
    ...
    private fun RecyclerView.createHeaderView(position: Int): View? {
        ...
        val holder = adapter
            .onCreateViewHolder(this, adapter.getItemViewType(position)) as? ViewHolder
            ?: return null
    ...
    private fun RecyclerView.findNextHeaderView(): View? {
        ...
        for (i in 1..visibleCount) {
            ...
            if (header != null && getChildViewHolder(header) as? ViewHolder != null) {
                return header
            }
        }
        return null
